...
struct ServoCommand_
{
  ...
  _header_type header;
   ...
   typedef std::vector<uint16_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint16_t>> _value_type;
  _value_type value;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hector_uav_msgs::ServoCommand_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.value.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.value[i]);
    }
    if (v.value.empty() || true)
      s << "]";
  }
