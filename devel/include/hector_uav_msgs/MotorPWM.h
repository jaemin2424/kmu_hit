...
struct MotorPWM_
{
  ...
  _header_type header;
   ...
   typedef std::vector<uint8_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint8_t>> _pwm_type;
  _pwm_type pwm;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hector_uav_msgs::MotorPWM_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.pwm.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.pwm[i]);
    }
    if (v.pwm.empty() || true)
      s << "]";
  }
