  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hector_uav_msgs::RC_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.axis.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.axis[i]);
    }
    if (v.axis.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "axis_function: ";
    if (v.axis_function.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.axis_function.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.axis_function[i]);
    }
    if (v.axis_function.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.swit.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.swit[i]);
    }
    if (v.swit.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "swit_function: ";
    if (v.swit_function.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.swit_function.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.swit_function[i]);
    }
    if (v.swit_function.empty() || true)
      s << "]";
  }
