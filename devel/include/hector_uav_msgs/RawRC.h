...
struct RawRC_
{
  ...
  _status_type status;
   ...
   typedef std::vector<uint16_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint16_t>> _channel_type;
...
bool operator==(const ::hector_uav_msgs::RawRC_<ContainerAllocator1> & lhs, const ::hector_uav_msgs::RawRC_<ContainerAllocator2> & rhs)
{
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hector_uav_msgs::RawRC_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.channel.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.channel[i]);
    }
    if (v.channel.empty() || true)
      s << "]";
  }
